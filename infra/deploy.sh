...
ACCESS_FLAGS=(--invoker-iam-check --min-instances=0)
if [[ "${PUBLIC_DEMO}" == "true" ]]; then
  ACCESS_FLAGS=(--no-invoker-iam-check --min-instances=1)
fi
...
if [[ "${PUBLIC_DEMO}" == "true" && "${VERIFY_DEPLOY:-true}" == "true" ]]; then
  BACKEND_URL="${BACKEND_URL}" FRONTEND_URL="${FRONTEND_URL}" \
    "${ROOT_DIR}/infra/verify.sh"
else
  echo "Private deployment complete. Run infra/enable-demo.sh before presenting."
fi
